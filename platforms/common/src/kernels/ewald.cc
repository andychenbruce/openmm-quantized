...
KERNEL void calculateEwaldCosSinSums(GLOBAL mixed* RESTRICT energyBuffer, GLOBAL const real4* RESTRICT posq, GLOBAL real2* RESTRICT cosSinSum, real4 periodicBoxSize) {
    ...
    real3 reciprocalBoxSize = make_real3(((real)(2*M_PI))/periodicBoxSize.x, ((real)(2*M_PI))/periodicBoxSize.y, ((real)(2*M_PI))/periodicBoxSize.z);
    real reciprocalCoefficient = ((real)(ONE_4PI_EPS0*4*M_PI))/(periodicBoxSize.x*periodicBoxSize.y*periodicBoxSize.z);
    ...
    while (index < totalK) {
        ...
        ry += -KMAX_Y + 1;
        real kx = ((real)rx)*reciprocalBoxSize.x;
        real ky = ((real)ry)*reciprocalBoxSize.y;
        real kz = ((real)rz)*reciprocalBoxSize.z;
        ...
        real ak = (real)EXP(k2*(real)EXP_COEFFICIENT) / k2;
        ...
        for (int rx = 0; rx < KMAX_X; rx++) {
            ...
            for (int ry = lowry; ry < KMAX_Y; ry++) {
                ...
                for (int rz = lowrz; rz < KMAX_Z; rz++) {
                    ...
                    real dEdR = (real)2*reciprocalCoefficient*ak*apos.w*(sum.x*structureFactor.y - sum.y*structureFactor.x);
                    force.x += dEdR*kx;
                    force.y += dEdR*ky;
                    force.z += dEdR*kz;
                    lowrz = 1 - KMAX_Z;
