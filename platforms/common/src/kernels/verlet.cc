...
KERNEL void integrateVerletPart1(int numAtoms, int paddedNumAtoms, GLOBAL const mixed2* RESTRICT dt, GLOBAL const real4* RESTRICT posq,
  ...
  // }
    ...
    const double scale = (double)dtVel/(double) (long long) 0x100000000;
    for (int index = GLOBAL_ID; index < numAtoms; index += GLOBAL_SIZE) {
	    ...
	    // printf("UUUUUU = %f\n", ((double)scale*(double)force[index]*(double)velocity.w));
	  
            velocity.x += (mixed)((double)scale*(double)force[index]*(double)velocity.w);
            velocity.y += (mixed)((double)scale*(double)force[index+paddedNumAtoms]*(double)velocity.w);
            velocity.z += (mixed)((double)scale*(double)force[index+paddedNumAtoms*2]*(double)velocity.w);
	    // printf("test = %f * %lld * %f\n", (double) scale, force[index], (double) velocity.w);
	    // //half test = scale*(mixed)force[index]*velocity.w;
	    // printf("uhhh = %f\n", (double)((mixed)(force[index]/100000)));
	    // printf("poo = %f\n", (double)CUDART_MAX_NORMAL_FP16);
